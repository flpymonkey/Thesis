...
class Algo(object):
    ...
    def run(self, S, n_jobs=1, log_progress=True) -> result.AlgoResult:
        ...
        if isinstance(S, result.ListResult):
            P = S.to_dataframe()
        else:
            P = S
        ...
        if isinstance(S, result.ListResult):
            ...
            return result.AlgoResult(S[0].X, B)
        else:
            return result.AlgoResult(self._convert_prices(S, "ratio"), B)
    ...
    def run_subsets(self, S, r, generator=False):
        ...
        if generator:
            ...
            for result, name in subset_generator():
                ...
                names.append(name)
            return result.ListResult(results, names)
